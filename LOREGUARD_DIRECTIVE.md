...
4. **저작권 가드레일 (코드 강제)**:
   ...
   - ~~게임 아트·스크린샷은 UI에 일절 사용하지 않는다.~~ **[개정 2026-08-06. 아래로 대체(superseded). 원문은 이력 보존을 위해 남긴다.]**
     게임 아트 에셋은 원칙적으로 사용하지 않는다. 예외: 공식 배포 채널(Steam 상점 페이지)의 프로모션 스크린샷은 연대기 화면에 한해 출처·저작권 표기와 함께 사용할 수 있다 (`/public/chronicle/`). 데이터마이닝·유출 이미지는 금지.
     **개정 사유**: 연대기가 첫 화면의 서사 축이 되면서 장별 시각 앵커가 필요해졌고, 권리자가 공개 배포 채널에서 홍보 목적으로 배포하는 이미지에 한해 출처·저작권·비공식 고지를 함께 표기하는 조건이면 나머지 저작권 가드레일과 충돌하지 않는다고 판단했다. **개정일 2026-08-06** · 근거 조항: `PRD §6.6′` · 구현 정책: `README.md` 저작권·데이터 윤리 절.
...
- `scripts/check.ts`: C1~C5 구현.
  ...
  - C4(톤): 톤 캐논 문서(`data/tone-canon.json`, 다크 판타지 어휘 원칙과 금지 패턴)를 먼저 작성하고 이를 기준으로 판정.
...
**금지**: 보라 그라디언트 SaaS 룩, 기본 shadcn 무가공 룩, 네온 아시드 단색 액센트 클리셰, 게임 원본 아트를 디자인 언어(배경·질감·컴포넌트)로 차용하는 것. 연대기 삽화는 §0.4 개정(2026-08-06)에 따른 예외이며, 디자인 시스템의 일부가 아니다.
...
**타이포그래피**:
- 디스플레이(제목·인장): **Noto Serif KR** (700/900). 획의 무게로 고문서 인상을 준다. 자간 -0.01em.
- 본문·UI: **Pretendard** (400/500/600).
- 데이터·ID·코드: **JetBrains Mono**. 팩트 ID, KB 버전, 신뢰도 라벨에 쓴다.
...
**구조 장치**: 팩트 카드는 상단에 `FACT-0017 · timeline · confirmed` 형태의 모노스페이스 아이브로우를 둔다. 장식이 아니라 데이터의 신원이다. 충돌 상태 카드는 좌측 2px ember 보더. superseded 카드는 취소선 없이 `→ 후속 팩트` 링크와 dim 처리(이력은 존중해서 표시).
...
- 파싱 실패 시 1회 재시도(포맷 교정 지시) 후 실패는 리포트에 `parse-error`로 기록한다. 조용히 삼키지 않는다.
